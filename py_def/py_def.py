...
def main():
    if (
        any([x in sys.argv for x in ["--help", "--h", "-help", "-h"]])
        or len(sys.argv) < 2
    ):
        print("usage: py-def [ file[.py]|dir ] 'some python code'")
    ...
    if len(sys.argv) == 2:
        pyloc_str = "."
        runcode = " ".join(sys.argv[1:])
    else:
        pyloc_str = sys.argv[1]
        runcode = " ".join(sys.argv[2:])
    ...
    for pyloc in pylocs:
        if os.path.isdir(pyloc):
            ...
            pyfiles.extend(
                [x for x in dir_items if os.path.isfile(x) and x.endswith(".py")]
            )
        else:
    ...
    for pyfile in pyfiles:
        ...
        module = __import__(
            fname[:-3] if fname.endswith(".py") else fname, globals(), locals(), ["*"]
        )
        for k in dir(module):
